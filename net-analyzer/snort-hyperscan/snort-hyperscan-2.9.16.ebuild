...
SRC_URI="https://www.snort.org/downloads/archive/snort/snort-${PV}.tar.gz"
LICENSE="GPL-2"
SLOT="0"
...
PATCHES=(
	"${FILESDIR}"/snort-2.9.8.3-no-implicit.patch
	"${FILESDIR}"/snort-2.9.8.3-rpc.patch
	"${FILESDIR}"/snort-2.9.12-snort.pc.patch
	"${FILESDIR}"/snort-2.9.15.1-fno-common.patch
	"${FILESDIR}"/snort-2982-hyperscan-v4.patch
)
...
src_configure() {
	econf \
		...
		$(use_enable hyperscan intel-hyperscan) \
		$(use_with libtirpc) \
		--enable-mpls \
		--enable-normalizer \
		--enable-reload \
		...
		--disable-static-daq \
		--with-intel-hyperscan-includes=/usr/include/hs \
		--with-intel-hyperscan-libraries=/usr/lib
}
...
src_install() {
	...
	# Configure Snort to use the Hyperscan MPSE matcher
	sed -i -e 's|config detection: search-method ac-split search-optimize max-pattern-len 20|config detection: search-method hyperscan split-any-any|g' \
		"${ED}/etc/snort/snort.conf.distrib" || die
}
